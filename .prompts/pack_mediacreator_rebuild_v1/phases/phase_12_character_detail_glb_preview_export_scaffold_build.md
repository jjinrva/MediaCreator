# Phase 12 build — Character detail route, GLB preview, and export scaffold

## Goal

Stand up the character detail screen with a truthful section layout and a GLB preview/export scaffold that will be refined in later phases.
...
- apps/web/components/glb-preview
...
### Step 1
Build the Phase 12 character detail page with these sections only: Overview, Body, Pose, History, Outputs. Do not expose wardrobe, scenes, or composer controls here yet.

### Step 2
Create a GLB preview component using the current `<model-viewer>` stack. The initial GLB may be a scaffolded base output from the Blender pipeline, but the preview must be real and API-backed.

### Step 3
Create an export job concept and output placeholder route so later Blender export phases have a stable destination. In this phase, truthful 'not ready' status is acceptable for the final Blender export if the backend artifact does not yet exist.

### Step 4
Ensure the viewer container is sized for full-body preview. Later phases will refine framing in detail.
...
4. `docs/phase_reports/phase_12.md` is updated with exact commands and results.
