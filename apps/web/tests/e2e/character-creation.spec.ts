...
test("persisted photoset creates one base character and reloads the detail route", async ({
  ...
  try {
    ...
    await expect(page.getByText("Preview generation job")).toBeVisible();
    await expect(page.getByText(/Worker (offline|stale):/)).toBeVisible({ timeout: 30000 });
    ...
    await expect(page.getByText("High-detail reconstruction job")).toBeVisible();
    await expect(
      page
        .locator(".keyValueRow")
    ...
    await expect(
      page.getByRole("img", { name: "Accepted reference male_head_front.png" })
    ).toBeVisible();
