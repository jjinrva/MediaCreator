...
type BodyParameterEditorProps = {
  characterPublicId: string;
  initialCatalog: BodyParameterCatalogEntry[];
...
export function BodyParameterEditor({
  characterPublicId,
  initialCatalog,
  initialValues
}: BodyParameterEditorProps) {
  ...
  async function handleCommit(parameterKey: string, numericValue: number, label: string, unit: string) {
    try {
      ...
      if (!response.ok) {
        throw new Error("Body parameter save failed.");
      }
      ...
      setError("Body parameter save failed before the canonical state could reload.");
    }
  }
