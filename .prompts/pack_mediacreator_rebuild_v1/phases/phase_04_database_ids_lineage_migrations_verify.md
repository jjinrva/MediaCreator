# Phase 04 verify — Database foundation, IDs, lineage, and migrations

## Verify goal

Prove that Phase 04 is working the way the build document required. Do not accept screenshots or vague statements as proof. Use commands, API assertions, UI assertions, file assertions, and database checks where needed.

## Mandatory verification order

1. Read `AGENTS.md`
2. Read the paired build file for Phase 04
...
6. Update `docs/verification/phase_04_verify.md`

## Exact verification steps

### Verify step 1
Run the first migration up and down successfully.

### Verify step 2
Assert UUID columns are present and generated correctly.

### Verify step 3
Insert and query a seeded actor record.

### Verify step 4
Create and query one sample asset and one history event through tests.

### Verify step 5
Confirm Alembic environment loads the full SQLAlchemy metadata correctly.

### Verify step 6
Confirm no parallel duplicate history or asset registry tables were created.
...
Write these sections into `docs/verification/phase_04_verify.md`:
- Scope verified
- Commands run
